...
CORE_Id BehaviorSetCreate(DT_void) {
    ...
    CORE_Id id = CORE_IdMgrAddData(g_state->behavior_sets, &b_set);
    if (id == CORE_INVALID_ID) {
        PRP_LOG_FN_CODE(PRP_FN_RES_EXHAUSTED_ERROR,
                        "Cannot create anymore BehaviorSets.");
    }
...
PRP_FnCode BehaviorSetDelete(CORE_Id *pB_set_id) {
    if (!pB_set_id) {
        PRP_LOG_FN_INV_ARG_ERROR(pB_set_id);
        return PRP_FN_INV_ARG_ERROR;
    }
    CORE_Id b_set_id = *pB_set_id;

    // We don't do id validation since the below function will do it anyways.
    return CORE_IdMgrDeleteData(g_state->behavior_sets, pB_set_id);
}
...
PRP_FnCode BehaviorSetClear(CORE_Id b_set_id) {
     ...
     * validity.
     */
    DT_Bitmap *b_set = CORE_IdToData(g_state->behavior_sets, b_set_id);
    if (!b_set) {
