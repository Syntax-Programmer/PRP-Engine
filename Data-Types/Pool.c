...
PRP_FN_API PRP_Pool *PRP_FN_CALL PRP_PoolCreate(PRP_size memb_size,
  ...
  if (!memb_size) {
    PRP_LOG_FN_CODE(PRP_FN_INV_ARG_ERROR,
                    "PRP_Pool can't be made with memb_size=0.");
    return PRP_null;
  }
  if (!cap) {
    PRP_LOG_FN_CODE(PRP_FN_INV_ARG_ERROR, "PRP_Pool can't be made with cap=0.");
    return PRP_null;
  }

  PRP_Pool *pool = malloc(sizeof(PRP_Pool) + (memb_size * cap));
  if (!pool) {
