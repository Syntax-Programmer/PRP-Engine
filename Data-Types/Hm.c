...
PRP_HmCreate(PRP_u64 (*hash_fn)(const PRP_void *key),
  ...
  memset(hm->layout, LAYOUT_EMPTYING_MASK, sizeof(PRP_size) * INIT_LAYOUT_CAP);
...
PRP_FN_API PRP_FnCode PRP_FN_CALL PRP_HmAdd(PRP_Hm *hm, PRP_void *key,
  ...
  while (hm->layout[i] != EMPTY_I) {
    if (hm->layout[i] == DEAD_I) {
      // Marking dead index for reuse. But searching fwd for key match check.
      j = i;
    } else {
...
PRP_FN_API PRP_FnCode PRP_FN_CALL PRP_HmDelElem(PRP_Hm *hm, PRP_void *key) {
  ...
  if (FetchLayoutElemI(hm, key, &key_layout_i, &key_elem_i) != PRP_FN_SUCCESS) {
